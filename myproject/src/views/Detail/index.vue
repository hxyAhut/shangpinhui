        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSale, index) in spuSaleAttrList" :key="index">
                <dt class="title">{{spuSale.saleAttrName}}</dt>
                <dd changepirce="0" :class="{active: spuSaleAttr.isChecked == 1}" @click="changeActive(spuSaleAttr,spuSale.spuSaleAttrValueList)" v-for="(spuSaleAttr, index) in spuSale.spuSaleAttrValueList" :key="spuSaleAttr.id">{{spuSaleAttr.id}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <!-- 输入框：聚焦、失焦，可以使用change事件 -->
                <input autocomplete="off" class="itxt" v-model="num" @change="changeNum">
                <a href="javascript:" class="plus" @click="num++">+</a>
                <a href="javascript:" class="mins" @click="num < 2 ? num = 1: num--">-</a>
              </div>
              <div class="add">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      changeActive(attrVal, attr) {
        console.log(attrVal, attr)
        attr.forEach(item => {
          item.isChecked = '0';
        })
        attrVal.isChecked = '1'
      ...
      // 加入购物车
      async addShopCart() {
        // 存储参数，通知服务器存储商品参数，即发请求，将参数带到服务器中
        try{
          let result = await this.$store.dispatch('addOrUpdateShopcart', {skuId: this.$route.params.skuid, skuNum: this.num})
          console.log(result)
          // 存储成功之后，进行路由跳转
          // 1.这种方式虽然也能在跳转路由之后拿到数据，但是路径非常乱
          // this.$router.push({name: 'addcartsuccess', query: this.skuInfo, skuNum: this.num})

          // 2.使用会话存储存储对象数据（复杂数据）,简单数据放在query中
          // 但会话存储和本地存储只能存储字符串，故须将对象转化为字符串
