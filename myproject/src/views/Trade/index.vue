<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(user, index) in users" :key="index">
        <span class="username" :class="{selected: user.isDefault == 1}" @click="changeUser(index)">{{user.consignee}}</span>
        <p>
    ...
    <div class="money clearFix">
      <ul>
        <li>
          <p>共<i>{{orderInfo.totalNum}}</i>件商品</p>
          <p>总商品金额：<span>¥{{totalPrice}}.00</span></p>
        </li>
        <li>
          <b>返现：</b>
          <span>0.00</span>
...
import {mapState} from 'vuex';
  export default {
    ...
    computed: {
      ...
      // 地址对应
      userAddress() {
        return this.users.find(item => item.isDefault == 1) || {}
      }
    },
    methods: {
      changeUser(index) {
      ...
      async submitOrder() {
        ...
        let data = {
          // 收件人姓名
          consignee: this.users.consignee,
          // 收件人电话
          consigneeTel: this.users.phoneNum,
          // 收件人地址
          deliveryAddress: this.users.fullAddress,
          // 支付方式
          paymentWay: "ONLINE",
          // 留言，备注
